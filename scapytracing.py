...
def monitor_network(log_file, filters, debug_mode=False, expire_time=60, excluded_processes=[], interface=None):
    ...
    # Build Berkeley Packet Filter (BPF) string based on filters
    bpf_filter = ""
    if 'TCP' in filters and 'UDP' not in filters:
        bpf_filter = "tcp"
    elif 'UDP' in filters and 'TCP' not in filters:
        bpf_filter = "udp"
    elif 'TCP' in filters and 'UDP' in filters:
        bpf_filter = "tcp or udp"
    
    callback = lambda pkt: packet_callback(pkt, process_port_dict, seen_connections, expire_time, filters)
    scapy.sniff(filter=bpf_filter, prn=callback, store=0, iface=interface)
...
def packet_callback(packet, process_port_dict, seen_connections, expire_time, filters):
    """Callback function to process each captured packet"""
    if IP in packet:
        # Check if packet matches the protocol filter
        if ('TCP' in filters and TCP not in packet) or ('UDP' in filters and UDP not in packet):
            return
        ...
        if TCP in packet:
            ...
            return  # If neither TCP nor UDP, exit the function
        
        # Get source and destination info
        local_addr = ip.src
        local_port = transport.sport
        remote_addr = ip.dst
        remote_port = transport.dport
        ...
        # Get protocol and connection type info
        protocol_info, process_info = get_protocol_info(packet, process_port_dict)
        conn_type = get_connection_type(packet)
        
        # Use process info from port mapping if available
